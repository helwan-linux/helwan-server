...
class MainWindow(QMainWindow):
	...
	def setup_ui(self):
		...
		# داخل main_window.py في دالة setup_ui
		self.kill_port_button = QPushButton("Kill Port")
		self.kill_port_button.setToolTip("Terminate any process using the selected port")
		# أضفه في الـ folder_layout أو الـ action_layout
		action_layout.addWidget(self.kill_port_button)

		# في دالة setup_connections
		self.kill_port_button.clicked.connect(self.kill_current_port)
	...
	def kill_current_port(self):
		port = self.current_port
		confirm = QMessageBox.question(self, "Confirm", f"Kill process on port {port}?", QMessageBox.Yes | QMessageBox.No)
		if confirm == QMessageBox.Yes:
			self.server.kill_port_process(port)
